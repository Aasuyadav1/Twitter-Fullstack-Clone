...
const Modal = ({showModel, setShowModal, toggleModel, postId }) => {
  ...
  const { initialState, setInitialState, getAllPosts } = useStore((state) => state);
            ...
            <div className="relative bg-[#192734] rounded-lg shadow ">
              <div className="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                ...
                <div className='flex gap-1 items-center'>
                  <img src={post?.createdBy?.avatar} className='h-[35px] rounded-full w-[35px] object-cover aspect-square' alt="" />
                  <div className='flex flex-col leading-0'>
                  <span className='text-white text-md'>{post?.createdBy?.name}</span>
                  <span className='text-gray-400 text-sm'>@{post?.createdBy?.username}</span>
                  </div>
              ...
              <div className="p-4 md:p-5 space-y-4">
               ...
               <textarea className='w-full outline-none bg-[#192734] rounded-lg p-2 text-white' name="content" value={formData?.content} id="" onChange={(e) => setformData({ ...formData, content: e.target.value })}></textarea>
